...
class Data_Visualizer():
    ...
    def visualize(self, s_total, v_avg, game_time):
        ...
        plt.ylabel('[units]')
        plt.plot(self.s_tots, color='blue', label='s [units]')
        plt.plot(self.vavg_tots, color='orange', label='av. v [units / s]')
        plt.plot(self.laptime_est, color='green', label='av. laptime (est.) [s]')
        plt.ylim(ymin=0)
